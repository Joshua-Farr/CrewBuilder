import { z } from "zod";
export const deckCardSchema = z.object({ cardId: z.string().min(1), quantity: z.coerce.number().int().min(1).max(4), category: z.enum(["Leader", "Character", "Event", "Stage", "Sideboard"]) });
